...
def trigger_heal():
    ...
    global emulator_instance, analyzer_module
    ...
    if not analyzer_module: return jsonify({"status": "error", "message": "Analyzer not configured"}), 500

    print(f"[Dashboard] Heal request received for: {issue_key}")
    # Call analyzer's function - main loop tracks pending state & confirmation
    success_attempted = analyzer_module.attempt_self_heal(issue_key, emulator=emulator_instance)
...
def send_diy_message():
    ...
    global diy_repair_instance, emulator_instance
    data = request.get_json()
    ...
    # Get current DTCs from emulator or use provided ones
    current_dtcs = []
    if emulator_instance and hasattr(emulator_instance, 'active_dtc') and emulator_instance.active_dtc:
        current_dtcs = [emulator_instance.active_dtc]
...
# --- Standalone Run ---
if __name__ == "__main__": print("[Dashboard] Running in standalone mode."); run_flask_app()